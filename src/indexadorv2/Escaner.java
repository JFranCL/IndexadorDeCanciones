...
public class Escaner {
    ...
    private void subeCancion(Canciones cancion) {
            ...
            if (compruebaCancion(cancion.getId())) {
    ...
    /**
     * Comprueba si la canción que se desea subir a la base de datos está
     * repetida
     *
     * @param id
     * @return false -> Está repetida || true -> No está en la bd
     */
    private boolean compruebaCancion(CancionesId id) {
    ...
    /**
     * Escribe en el archivo Log.txt, la acción que ha realizado, la fecha y
     * hora.
     *
    ...
    /**
     * Localiza todos los generos disponibles en la base de datos
     *
     * @return Lista de los generos disponibles
     */
    public ArrayList localizaGeneros(ArrayList cancionesDisponibles) {
        ArrayList resultado = new ArrayList();
        Canciones c;

        //Añadimos a resultado el genero de cada canción, siempre que no sea null
        for (int i = 0; i < cancionesDisponibles.size(); i++) {
            c = (Canciones) cancionesDisponibles.get(i);
            if (c.getGenero() != null) {
                resultado.add(c.getGenero());
            }
        }

        //Eliminamos los Generos repetidos
        Set<String> hashSet = new HashSet<String>(resultado);
        resultado.clear();
